...
- Domain đã trỏ về IP VPS qua **Cloudflare** (A record, Proxy OFF khi dùng SSL trực tiếp)
...
> ⚠️ Mỗi lần SSH mới cần chạy lại `export NVM_DIR...` hoặc thêm vào `~/.bashrc`

---

## BƯỚC 8 — SSH: Clone Repo và Cấu Hình
...
Mỗi project Next.js cần **một port riêng**. Các port thường dùng:
...
Kiểm tra port đang dùng: `ss -tlnp`
...
> ⚠️ Thay `nodejs3005` và `3005` bằng port thực tế của project, thay `<domain>` bằng domain thật.

4. Save → Restart LiteSpeed trong SSH:
...
| suckhoetaman.com | suckh2097 | 3001 | (xem server) | (xem server) |
...
- **Port không được trùng**: Mỗi project một port riêng
- **prisma/schema.prisma**: Đảm bảo file này có trong repo trước khi chạy `prisma migrate deploy`
- **Cloudflare Proxy**: Bật cam (Proxied) để có SSL qua Cloudflare, hoặc tắt nếu dùng Let's Encrypt trực tiếp
